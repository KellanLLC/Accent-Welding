...
|---|---|---|---|
| 1 | **Which phone leads?** `801-691-3685` (business card + price list) or `385-241-4679` (both IG flyers)? | `src/config/business.ts` → `phone` / `phoneAlt` | 801 leads everywhere; 385 shows as the second line in the footer and on Contact. |
...
## 3. Where quote submissions go

`POST /api/quote` validates the payload and appends it to `site/.data/quotes.jsonl`,
so nothing is lost, and logs it to the server console. **No email is sent yet.**

To turn on email, put a key in `site/.env.local` and un-comment the block in
...
The rest of the route needs no changes.
...
src/components/    Nav, Footer, Hero, Mark, Lockup, controls/, builder/, page/
src/app/           routes; build/* are the four configurators
```
